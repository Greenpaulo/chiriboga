...
This is a proposed sibling to `documentation/corp_ai_improvement_roadmap.md` and `documentation/corp_ai_install_decision_roadmap.md`, extending the same architecture work to the Runner side, which has not yet received an equivalent pass. None of the phases in this document are implemented unless their status is changed explicitly after code, documentation, and regression tests have been completed.
...
- the mulligan decision (currently gated on whether *any* card in the opening hand is judged worth keeping);
...
6. update `documentation/ai.md` only if a card-facing hook contract changes (it shouldn't in Phase 0/1 — existing hooks are being *read* differently, not redefined);
