...
<head>
  ...
  <?php
  echo '<link rel="stylesheet" type="text/css" href="style.css?' . filemtime('style.css') . '" />';
  $jsfiles = array('init', 'phase', 'command', 'checks', 'mechanics', 'utility', 'config', 'sounds');
...
<body id="body" onload="Init();">
  <script>
    ...
    function toggleCrtFromMenu(enabled) {
      ...
      try {
        ...
        p.crtEffects = enabled;
        localStorage.setItem('chiriboga-settings', JSON.stringify(p));
      } catch (e) {}
    }
    document.addEventListener('DOMContentLoaded', function() {
      try {
        ...
        if (s) {
          var p = JSON.parse(s);
          var cb = document.getElementById('crt-toggle');
          if (cb) cb.checked = (p.crtEffects !== false);
        }
      } catch (e) {}
    });
  </script>
  ...
  <div id="loading" class="modal" style="display:flex;">
    <div class="modal-content-inactive">
      <h1 id="loading-text">DECKBUILDING...</h1>
    </div>
  </div>
