...
(function () {
  ...
  JSON.stringify = function (message, setNumbers = false) {
    //if setNumbers is true, the card's set number will be output instead of title
...
function Readablify(message) {
  return (
    JSON.stringify(message).replaceAll('\\"', "").replaceAll('"', "") + "\n"
  );
}
var capturedLog = [];
(function () {
...
function unlockAchievement(achievementId) {
  try {
    ...
    for (var i = 0; i < data.achievements.length; i++) {
      if (
        data.achievements[i].id === achievementId &&
        !data.achievements[i].achieved
      ) {
...
function updateGauntletAchievements(
  score,
  identity,
  gauntletLength,
  isComplete,
...
function LocationStringIfRelevant(src, cardprop = "") {
  if (src.cardLocation) {
    ...
    if (src.host) {
      ...
      if (idx > -1)
        return LocationStringIfRelevant(src.host) + ".hostedCards[" + idx + "]";
    }
    //list of locations:
    var locations = [
    ...
    for (var i = 0; i < corp.remoteServers.length; i++) {
      locations.push({
        ary: corp.remoteServers[i].root,
        str: "corp.remoteServers[" + i + "].root",
      });
      locations.push({
        ary: corp.remoteServers[i].ice,
        str: "corp.remoteServers[" + i + "].ice",
      });
    }
  ...
  if (src.isCard) {
    ...
    if (cardprop != "")
      console.error(
        "Converting " + cardprop + "=" + src.title + " to string not supported",
      );
    else
      console.error(
        "Converting property=" + src.title + " to string not supported",
      );
  }
  return src;
}
...
function ValueToString(val, reportErrors = false, prop = "", cardprop = "") {
  ...
  if (reportErrors) {
    ...
    console.error(
      "Value above" + propstr + " is unsupported in ValueToString.",
    );
  }
  return "";
}
  ...
  for (var j = 0; j < src.length; j++) {
    ...
    //runner special handled properties
      ...
      if (card.faceUp) {
        ...
        if (
          !runner.rig.programs.includes(card) &&
    ...
    for (var i = 0; i < counterList.length; i++) {
      if (!handledProps.includes(counterList[i])) {
        if (typeof card[counterList[i]] !== "undefined") {
          if (card[counterList[i]] != 0)
            ret +=
              addr + "." + counterList[i] + "=" + card[counterList[i]] + ";\n";
        }
    ...
    for (var i = 0; i < cardPropertyResets.length; i++) {
      ...
      if (!handledProps.includes(propName)) {
        if (typeof card[propName] !== "undefined") {
          if (card[propName] != cardPropertyResets[i].defaultValue)
            ret +=
              addr +
              "." +
              propName +
              "=" +
              LocationStringIfRelevant(
                card[propName],
                card.title + "." + propName,
              ) +
              ";\n";
    ...
    if (card.hostedCards && !handledProps.includes("hostedCards")) {
      ...
      for (var i = 0; i < card.hostedCards.length; i++) {
        ret +=
          "InstanceCardsPush(" +
          card.hostedCards[i].setNumber +
          "," +
          addr +
          ".hostedCards,1,cardBackTextures" +
          PlayerName(card.hostedCards[i].player) +
          ",glowTextures,strengthTextures)[0].host = " +
          addr +
          ";\n";
      }
...
function ReproductionCode(full = false) {
  ...
  var runnerInstalled = JSON.stringify(
    runner.rig.resources
      .concat(runner.rig.hardware)
      .concat(runner.rig.programs),
    true,
  );
  var runnerStolen = JSON.stringify(runner.scoreArea, true);
  ret +=
    "RunnerTestField(" +
    runner.identityCard.setNumber +
    ", " +
    [runnerHeap, runnerStack, runnerGrip, runnerInstalled, runnerStolen].join(
      ", ",
    ) +
  ...
  if (runner.identityCard.setAsideCards) {
    ...
    for (var i = 0; i < card.setAsideCards.length; i++) {
      ret +=
        "InstanceCardsPush(" +
        card.setAsideCards[i].setNumber +
        "," +
        addr +
        ".setAsideCards,1,cardBackTextures" +
        PlayerName(card.setAsideCards[i].player) +
        ",glowTextures,strengthTextures)[0].host = " +
        addr +
        ";\n";
    }
  ...
  var corpArchivesInstalled = JSON.stringify(
    corp.archives.root.concat(corp.archives.ice),
    true,
  );
  var corpRnDInstalled = JSON.stringify(
    corp.RnD.root.concat(corp.RnD.ice),
    true,
  );
  var corpHQInstalled = JSON.stringify(corp.HQ.root.concat(corp.HQ.ice), true);
  var corpRemotesEach = [];
  for (var i = 0; i < corp.remoteServers.length; i++) {
    corpRemotesEach.push(
      JSON.parse(
        JSON.stringify(
          corp.remoteServers[i].root.concat(corp.remoteServers[i].ice),
          true,
        ),
      ),
    );
  ...
  ret +=
    "CorpTestField(" +
    corp.identityCard.setNumber +
    ", " +
    [
      corpArchivesCards,
      corpRndCards,
      corpHQCards,
      corpArchivesInstalled,
      corpRnDInstalled,
      corpHQInstalled,
      corpRemotes,
      corpScored,
  ...
  if (runner.identityCard.setAsideCards)
    ret += ReplicationCode(
      runner.identityCard.setAsideCards,
      "runner.identityCard.setAsideCards",
    );
  //corp arrays
  ...
  for (var i = 0; i < corp.remoteServers.length; i++) {
    ret += ReplicationCode(
      corp.remoteServers[i].root,
      "corp.remoteServers[" + i + "].root",
    );
    ret += ReplicationCode(
      corp.remoteServers[i].ice,
      "corp.remoteServers[" + i + "].ice",
    );
    if (typeof corp.remoteServers[i].AISuccessfulRuns !== "undefined")
      ret +=
        "corp.remoteServers[" +
        i +
        "].AISuccessfulRuns=" +
        corp.remoteServers[i].AISuccessfulRuns +
        ";\n";
  }
  //and now other properties
  if (typeof corp.archives.AISuccessfulRuns !== "undefined")
    ret +=
      "corp.archives.AISuccessfulRuns=" +
      corp.archives.AISuccessfulRuns +
      ";\n";
  if (typeof corp.RnD.AISuccessfulRuns !== "undefined")
    ret += "corp.RnD.AISuccessfulRuns=" + corp.RnD.AISuccessfulRuns + ";\n";
  if (typeof corp.HQ.AISuccessfulRuns !== "undefined")
    ret += "corp.HQ.AISuccessfulRuns=" + corp.HQ.AISuccessfulRuns + ";\n";
  if (runner.AI && typeof runner.AI.suspectedHQCards !== "undefined") {
    ...
    for (var i = 0; i < runner.AI.suspectedHQCards.length; i++) {
      ret +=
        "{title:'" +
        runner.AI.suspectedHQCards[i].title +
        "',cardType:'" +
        runner.AI.suspectedHQCards[i].cardType +
        "',copies:" +
        runner.AI.suspectedHQCards[i].copies +
        ",uncertainty:" +
        runner.AI.suspectedHQCards[i].uncertainty +
        "},";
    }
    ret += "];\n";
  }
  if (full || corp.badPublicity > 0)
    ret += "corp.badPublicity = " + corp.badPublicity + ";\n";
  if (full || runner.tags > 0) ret += "runner.tags = " + runner.tags + ";\n";
  if (full || runner.coreDamage > 0)
    ret += "runner.coreDamage = " + runner.coreDamage + ";\n";
  if (full || corp.tempBonusClicks > 0)
    ret += "corp.tempBonusClicks = " + corp.tempBonusClicks + ";\n";
  if (full || runner.tempBonusClicks > 0)
    ret += "runner.tempBonusClicks = " + runner.tempBonusClicks + ";\n";
  if (full) {
    ...
    for (var i = 0; i < phasesKeys.length; i++) {
      if (phases[phasesKeys[i]] == currentPhase)
        ret += "ChangePhase(phases." + phasesKeys[i] + "); ";
    }
...
function AutoContinueButtonHTML(showEvenIfOff = false) {
  if (
    (!autoContinue && !showEvenIfOff) ||
    (runner.AI && corp.AI) ||
    specifiedMentor != ""
  )
    return "";
  return (
    '<button style="font-size:80%; position:fixed; left:100%; top:-6px; height:67px;" id="autocontinue" title="Automatically continue" class="button autocontinue" onclick="autoContinue = !autoContinue; autoContinueTimer=0.0; $(\'#autocontinue\').html(AutoContinueButtonStr());">' +
    AutoContinueButtonStr() +
    "</button>"
  );
}
...
function DownloadCapturedLog() {
  ...
  for (var i = 0; i < corp.remoteServers.length; i++) {
    extraOutput +=
      "SPOILER: " +
      ServerName(corp.remoteServers[i]) +
      ": " +
...
function Narrate() {
  if ($("#narration").prop("checked") && stackedLog.length > 0) {
    ...
    //Runner
    src = src.replace(
      /Runner spent(.*?), Runner spent (\S*)/gm,
      function (match, p1, p2, offset, string) {
        return "Runner spent" + p1 + " and " + p2;
      },
    );
    src = src.replace(
      /Runner spent(.*?), Played/gm,
      function (match, p1, offset, string) {
        return "Runner spent" + p1 + " to play";
      },
    );
    src = src.replace(
      /Runner spent(.*?), Runner (\S*)/gm,
      function (match, p1, p2, offset, string) {
    ...
    src = src.replace(
      /Runner spent(.*?), Run initiated attacking/gm,
      function (match, p1, offset, string) {
    ...
    src = src.replace(
      /spent(.*), ([0-9]*) (credit|credits) taken/gm,
      function (match, p1, p2, p3, offset, string) {
        return "spent" + p1 + " to take " + p2 + " " + p3;
      },
    );
    src = src.replace(
      /spent(.*), ([0-9]*) (credit|credits) placed/gm,
      function (match, p1, p2, p3, offset, string) {
...
function Log(src) {
  ...
  if (activePlayer) {
    if (
      $("#narration").prop("checked") &&
      (activePlayer.AI || accessibilityMode == "text")
    )
      stackedLog.push(src); //only narrate AI player
  }
}
...
function GetServer(card) {
  ...
  for (var i = 0; i < corp.remoteServers.length; i++) {
    serverArrays.push({
      server: corp.remoteServers[i],
      array: corp.remoteServers[i][prop],
    });
  }
  for (var i = 0; i < serverArrays.length; i++) {
    if (card.cardLocation == serverArrays[i].array)
      return serverArrays[i].server;
  }
  return null;
}
...
function ShowGauntletRecap(gauntletState) {
  ...
  var creditScoreDivisor =
    typeof gauntletConfig !== "undefined" &&
    gauntletConfig.matchRewards &&
  ...
  var gauntletLength = gauntletState.opponents
    ? gauntletState.opponents.length
    : 0;
  ...
  var opponentListHtml =
    '<div class="gauntlet-recap-opponents" style="overflow-y: auto; max-height: 400px; padding: 10px 0;">';
  ...
  if (gauntletState.opponents && gauntletState.opponents.length > 0) {
    for (var i = 0; i < gauntletState.opponents.length; i++) {
      ...
      var identityTitle = identityCard
        ? GetTitle(identityCard)
        : "Unknown Identity";
      ...
      if (
        typeof opponent.startingPerk === "number" &&
        opponent.startingPerk > 0
      ) {
      ...
      var imageFile =
        identityCard && identityCard.imageFile ? identityCard.imageFile : "";
      ...
      var itemStyle =
        "display: flex; align-items: center; margin: 8px 0; padding: 5px; border-bottom: 1px solid rgba(51, 255, 51, 0.2);";
      var onclickHandler = "";
      if (deckUrl) {
      ...
      opponentListHtml +=
        '<div class="gauntlet-opponent-item" style="' +
        itemStyle +
        '"' +
        onclickHandler +
        ">";
      ...
      if (imageSrc) {
        opponentListHtml +=
          '<img src="' +
          imageSrc +
          '" style="width: 40px; height: 56px; object-fit: cover; margin-right: 10px; border: 1px solid #33ff33;" alt="' +
          identityTitle +
          '" />';
      } else {
        opponentListHtml +=
          '<div style="width: 40px; height: 56px; margin-right: 10px; border: 1px solid #33ff33; background: #111; display: flex; align-items: center; justify-content: center; font-size: 10px; color: #33ff33;">No Image</div>';
      }
      ...
      opponentListHtml +=
        ...
        "</div>";
      if (perkName) {
        var perkStyle = perkDisabled
          ? "color: #ff6666; text-decoration: line-through;"
          : "color: #ffcc00;";
        opponentListHtml +=
  ...
  var recapHtml =
    '<div class="solo-menu" style="display: flex; flex-direction: column; align-items: center;">';
  recapHtml +=
    '<span class="menu-close" onclick="window.location.href=\'index.php\';" style="cursor: pointer; align-self: flex-end; margin-right: 10px; margin-top: 10px;">✕</span>';
  recapHtml += '<div class="solo-logo" style="width: 100%;">';
  recapHtml += '<h1 class="logo-text">GAUNTLET COMPLETE</h1>';
  recapHtml += "</div>";
  recapHtml +=
    '<div style="color: #33ff33; font-family: monospace; padding: 20px; text-align: center; width: 100%;">';
  recapHtml +=
    '<div style="margin-bottom: 20px; font-size: 16px;">You have defeated these decks:</div>';
  recapHtml += opponentListHtml;
  recapHtml +=
    '<div style="margin-top: 20px; padding-top: 20px; border-top: 2px solid #33ff33; font-size: 18px;">';
  recapHtml +=
    '<div style="color: #ffff00; font-weight: bold;">SCORE: ' +
    score +
    "</div>";
  if (gauntletState.strictPacks && strictPacksPenalty > 0) {
    recapHtml +=
      '<div style="color: #ff9900; font-size: 12px; margin-top: 5px;">(Strict Packs: -' +
      strictPacksPenalty +
      ")</div>";
  }
  recapHtml += "</div>";
  recapHtml +=
    '<div style="display: flex; justify-content: center; margin-top: 20px;"><button class="button" onclick="window.location.href=\'index.php\';">RETURN TO MENU</button></div>';
  recapHtml += "</div>";
  recapHtml += "</div>";
...
function ShowGauntletLostModal(gauntletState) {
  ...
  var agendaPointStolenReward =
    typeof gauntletConfig !== "undefined" &&
    gauntletConfig.matchRewards &&
    typeof gauntletConfig.matchRewards.agendaPointStolen !== "undefined"
      ? gauntletConfig.matchRewards.agendaPointStolen
      : 3;
  var agendaPointScoredReward =
    typeof gauntletConfig !== "undefined" &&
    gauntletConfig.matchRewards &&
    typeof gauntletConfig.matchRewards.agendaPointScored !== "undefined"
      ? gauntletConfig.matchRewards.agendaPointScored
      : -2;

  // Calculate credits earned/lost from this game's agendas
  var agendaCreditsThisGame =
    agendaPointStolenReward * agendaStolenThisGame +
    agendaPointScoredReward * agendaScoredThisGame;
  ...
  console.log(
    "agendaStolen (previous): " +
      agendaStolenPrev +
      " + (this game): " +
      agendaStolenThisGame +
      " = +" +
      totalAgendaStolen,
  );
  console.log(
    "agendaScored (previous): " +
      agendaScoredPrev +
      " + (this game): " +
      agendaScoredThisGame +
      " = -" +
      totalAgendaScored,
  );
  console.log(
    "credits: " +
      ...
      agendaStolenThisGame +
      " × " +
      agendaPointStolenReward +
      ") + (" +
      agendaScoredThisGame +
      " × " +
      agendaPointScoredReward +
      ") = " +
      baseCredits +
      " + " +
      agendaCreditsThisGame +
      " = " +
      totalCredits,
  );
  console.log(
    ...
    for (var i = 0; i < gauntletState.opponents.length; i++) {
      ...
      if (imageSrc) {
        opponentListHtml +=
          '<img src="' +
          imageSrc +
          '" style="width: 40px; height: 56px; object-fit: cover; margin-right: 10px; border: 1px solid #33ff33;' +
          (isDefeated ? "" : " filter: grayscale(80%);") +
          '" alt="' +
          identityTitle +
          '" />';
      } else {
  ...
  var defeatedCount = gauntletState.defeated || 0;
  var totalOpponents = gauntletState.opponents
    ? gauntletState.opponents.length
    : 0;
  ...
  lostHtml +=
    '<h1 class="logo-text" style="color: var(--crt-red); text-shadow: 0 0 5px var(--crt-red), 0 0 15px var(--glow-red), 0 0 35px var(--glow-red-dark);">GAUNTLET LOST</h1>';
  lostHtml += "</div>";
  lostHtml +=
    '<div style="color: #33ff33; font-family: monospace; padding: 20px; text-align: center; width: 100%;">';
  lostHtml +=
    '<div style="margin-bottom: 20px; font-size: 16px;">Progress: ' +
    defeatedCount +
    " / " +
    totalOpponents +
    " opponents defeated</div>";
  ...
  lostHtml +=
    '<button class="button" onclick="window.location.href=\'index.php\';">RETURN TO MENU</button>';
  lostHtml += "</div>";
  lostHtml += "</div>";
  lostHtml += "</div>";
  ...
  */
  ...
  if (typeof gtag !== "undefined" && window.location.hostname !== "localhost") {
    ...
    if (gameMode === "Regular") {
      try {
        ...
        if (corpDeckParam !== "") {
          var corpDeck = JSON.parse(
            LZString.decompressFromEncodedURIComponent(corpDeckParam),
          );
      ...
      try {
        ...
        if (runnerDeckParam !== "") {
          var runnerDeck = JSON.parse(
            LZString.decompressFromEncodedURIComponent(runnerDeckParam),
          );
    ...
    if (gameMode === "Gauntlet") {
      try {
        ...
        var gauntletState = JSON.parse(
          LZString.decompressFromEncodedURIComponent(gauntletParam),
        );
        if (gauntletState) {
  ...
  if (isGauntletMode) {
    ...
    if (winner == runner) {
      ...
      winPhase.Resolve["Continue Gauntlet"] = function () {
        try {
          ...
          if (
            typeof currentOpponentIndex === "number" &&
            gauntletState.opponents &&
            gauntletState.opponents[currentOpponentIndex]
          ) {
            gauntletState.opponents[currentOpponentIndex].hasbeendefeated =
              true;
            console.log(
              "Marked opponent " + currentOpponentIndex + " as defeated",
            );
            ...
            console.log(
              "Added opponent " + currentOpponentIndex + " to defeatOrder:",
              gauntletState.defeatOrder,
            );
          }
          ...
          gauntletState.agendaScored =
            (gauntletState.agendaScored || 0) + AgendaPoints(corp);

          // Add runner's agenda points to total (agendas stolen by runner)
          gauntletState.agendaStolen =
            (gauntletState.agendaStolen || 0) + AgendaPoints(runner);
          ...
          if (
            ...
            try {
              ...
              var victory =
                typeof rewards.victory !== "undefined" ? rewards.victory : 5;
              var agendaPointStolen =
                typeof rewards.agendaPointStolen !== "undefined"
                  ? rewards.agendaPointStolen
                  : 0;
              var agendaPointScored =
                typeof rewards.agendaPointScored !== "undefined"
                  ? rewards.agendaPointScored
                  : 0;
              var bossBeatenReward =
                typeof rewards.bossBeaten !== "undefined"
                  ? rewards.bossBeaten
                  : 0;
              var minimalCredits =
                typeof rewards.minimalCredits !== "undefined"
                  ? rewards.minimalCredits
                  : 0;
              ...
              var opponentNumber =
                typeof currentOpponentIndex === "number"
                  ...
                  : 0;
              var isBossOpponent =
                opponentNumber > 0 && opponentNumber % 4 === 0;
              ...
              if (floorApplied) {
                textLines.push(
                  "(Minimum of " + minimalCredits + " credits applied)",
                );
              }
              textLines.push("Total: " + creditsThisMatch + " credits");

              gauntletState.creditsWonText = textLines.join("\n");
              gauntletState.creditsWon =
                (gauntletState.creditsWon || 0) + creditsThisMatch;
              console.log(
                "Credits won this match: " +
                  creditsThisMatch +
                  " (victory:" +
                  victory +
                  " boss:" +
          ...
          var updatedGauntlet = LZString.compressToEncodedURIComponent(
            JSON.stringify(gauntletState),
          );
          ...
          if (gauntletState.opponents && gauntletState.opponents.length > 0) {
            ...
            var opponentIndex =
              gauntletState.defeated % gauntletState.opponents.length;
            var nextOpponent = gauntletState.opponents[opponentIndex];
            // Encode the next opponent deck
            nextCorpDeck = LZString.compressToEncodedURIComponent(
              JSON.stringify(nextOpponent),
            );
          }
    ...
    winPhase.Resolve["edit deck"] = function () {
      ...
      var runnerDeckObj = {
        identity: runner.identityCard.setNumber,
        cards: [],
      };
      ...
      allRunnerCards = allRunnerCards.concat(
        runner.rig.programs,
        runner.rig.hardware,
        runner.rig.resources,
      );
      ...
      var allCorpCards = corp.RnD.cards.concat(
        corp.HQ.cards,
        corp.archives.cards,
      );
      for (var i = 0; i < corp.remoteServers.length; i++) {
      ...
      allCorpCards = allCorpCards.concat(
        corp.HQ.root,
        corp.HQ.ice,
        corp.RnD.root,
        corp.RnD.ice,
      ...
      var runnerCompressed = LZString.compressToEncodedURIComponent(
        JSON.stringify(runnerDeckObj),
      );
      var corpCompressed = LZString.compressToEncodedURIComponent(
        JSON.stringify(corpDeckObj),
      );
      ...
      window.location.href =
        "decklauncher.php?p=r&r=" + runnerCompressed + "&c=" + corpCompressed;
    };
  }
  ...
  Log(
    "Corp (" +
      corp.identityCard.title +
      ") agenda points: " +
      AgendaPoints(corp),
  );
  Log(
    "Runner (" +
      runner.identityCard.title +
      ") agenda points: " +
      AgendaPoints(runner),
  );
  ...
  console.log(
    "Agendas were stolen from: " + JSON.stringify(agendaStolenLocations),
...
function PlayerCanLook(player, card) {
  ...
  if (
    typeof player.identityCard.setAsideCards != "undefined" &&
    card.cardLocation == player.identityCard.setAsideCards
  )
    return true;
  if (
    player == corp &&
...
function ResetClicks(player) {
  //DRBO6 - Edited this to accommodate Aggressive Trendsetting
...
function ResolveAccess() {
  ...
  if (accessingCard.cardLocation == corp.RnD.cards)
    accessingCard.renderer.sprite.visible = false;
...
function InstalledMemoryCost(destination = null, ignoreCards = []) {
  ...
  for (var i = 0; i < arrayToCheck.length; i++) {
    if (typeof arrayToCheck[i].memoryCost !== "undefined")
      if (!ignoreCards.includes(arrayToCheck[i]))
...
function MoveCardTriggers(card, locationfrom, locationto) {
  ...
  if (locationfrom !== null) {
    ...
    if (runner.AI && attackedServer) {
      if (locationfrom == attackedServer.root)
  ...
  for (var i = 0; i < triggerList.length; i++) {
    ...
    if (!callbackPushed && secondCallbackName != "") {
      if (CheckCallback(triggerList[i].card, secondCallbackName)) {
        ...
        var choices = [{}]; //assume valid by default
        if (
          typeof triggerList[i].card[secondCallbackName].Enumerate ===
          "function"
        )
...
function ChoicesSubroutine(iceCard, triggeredSubroutine) {
  var ret = [{ card: iceCard, ability: triggeredSubroutine, choice: null }]; //subroutines fire even if there are no choices to be made
...
function ChoicesEncounteredSubroutines() {
  ...
  if (
    !attackedServer ||
    !attackedServer.ice ||
    !attackedServer.ice[approachIce]
  ) {
    return ret;
  }
  for (var i = 0; i < attackedServer.ice[approachIce].subroutines.length; i++) {
    ...
    if (!subroutine.broken && !subroutine._lockedFromBreak) {
      var params = {};
  ...
  if (typeof card[abilitiesProperty] !== "undefined") {
    ...
    for (var i = 0; i < card[abilitiesProperty].length; i++) {
      ...
      if (acceptable && choices.length > 0) {
        ...
        if (typeof card[abilitiesProperty][i].alt != "undefined")
          params.alt = card[abilitiesProperty][i].alt; //if buttons are required
  ...
  if (CheckRez(card, validTypes)) {
    ...
    if (
      ...
      var reducedCost = Math.max(
        0,
        currentRezCost - card.optionalForfeitRezReduction,
      );
 ...
 */
function TriggeredResponsePhase(
  player,
  callbackName,
  enumerateParams,
  afterOpportunity,
  ...
  if (ChoicesActiveTriggers(callbackName).length > 0)
    wouldTriggerNothing = false;
  else if (
    secondCallbackName != "" &&
    ChoicesActiveTriggers(secondCallbackName).length > 0
  )
    wouldTriggerNothing = false;
 ...
 * @returns {Phase} the pseudophase created
 */
function OpportunityForAvoidPrevent(
  player,
  callbackName,
  enumerateParams,
  afterOpportunity,
...
function LegalCardForDeckBuild(
  ...
  if (
    ...
    cardSet[cardNumber].influence == 0
  ) {
    var influenceThatWillBeLeft = maxInfluence - totalInfluence;
    if (
      !exactInfluence ||
      influenceThatWillBeLeft == 0 ||
      cardSlotsThatWillBeLeft > 0
    )
      return true;
  } else {
    var infFlex = 2;
    ...
    if (exactInfluence) {
      infFlex = 0;
      var influenceThatWillBeLeft =
        maxInfluence - totalInfluence - cardSet[cardNumber].influence;
      ...
      if (
        cardSlotsThatWillBeLeft > influenceThatWillBeLeft &&
        cardSet[cardNumber].influence > 0
      )
        suitableInfluence = false; //don't use up all the influence immediately (card pool may not have enough in-faction cards to complete the quota otherwise)
      else if (
        cardSlotsThatWillBeLeft == 0 &&
        influenceThatWillBeLeft > 0 &&
        cardSet[cardNumber].influence < highestInfInPool
      )
        suitableInfluence = false; //try not to waste any influence
    }
    if (
      totalInfluence + cardSet[cardNumber].influence <=
        maxInfluence + infFlex &&
      suitableInfluence
    )
      return true;
  }
...
function DeckBuildRandomly(
  ...
  for (var i = 0; i < indices.length; i++) {
    if (
      cardSet[indices[i]].faction != identityCard.faction &&
      cardSet[indices[i]].influence > highestInfInPool &&
      !maxedIndices.includes(i)
    )
      highestInfInPool = cardSet[indices[i]].influence;
  }
...
function DeckBuild(
  ...
  if (identityCard.player == runner) {
    ...
    //consoles
    var consoleCards = [];
    if (setIdentifiers.includes("sg"))
      consoleCards = consoleCards.concat([30003, 30023, 30014]);
    if (setIdentifiers.includes("su21")) consoleCards = consoleCards.concat([]);
    if (setIdentifiers.includes("ms"))
      consoleCards = consoleCards.concat([33006]);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        consoleCards,
        destination,
    ...
    var fracterCards = [];
    if (setIdentifiers.includes("sg"))
      fracterCards = fracterCards.concat([30006, 30016]);
    if (setIdentifiers.includes("su21"))
      fracterCards = fracterCards.concat([31006]);
    if (setIdentifiers.includes("ms"))
      fracterCards = fracterCards.concat([33007]);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        fracterCards,
    ...
    var decoderCards = [];
    if (setIdentifiers.includes("sg"))
      decoderCards = decoderCards.concat([30005, 30026]);
    if (setIdentifiers.includes("su21"))
      decoderCards = decoderCards.concat([31021, 31033]);
    var numDecoders = RandomRange(1, 2);
    if (identityCard.title == 'Rielle "Kit" Peddler: Transhuman')
      numDecoders = RandomRange(2, 3); //special case
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        decoderCards,
    ...
    var killerCards = [];
    if (setIdentifiers.includes("sg"))
      killerCards = killerCards.concat([30015, 30025]);
    if (setIdentifiers.includes("su21"))
      killerCards = killerCards.concat([31008, 31022]);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        killerCards,
    ...
    if (setIdentifiers.includes("sg"))
      creditEconomyCards = creditEconomyCards.concat([
        30007, 30018, 30020, 30027, 30029, 30030, 30033,
      ]);
    if (setIdentifiers.includes("su21"))
      creditEconomyCards = creditEconomyCards.concat([
        31010, 31011, 31015, 31024, 31034, 31035, 31037, 31038,
      ]);
    if (setIdentifiers.includes("ms"))
      creditEconomyCards = creditEconomyCards.concat([33005]);
    var influenceUsed = CountInfluence(identityCard, cardsAdded);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        creditEconomyCards,
    ...
    if (setIdentifiers.includes("sg"))
      drawEconomyCards = drawEconomyCards.concat([30002, 30011, 30021, 30034]);
    if (setIdentifiers.includes("su21"))
      drawEconomyCards = drawEconomyCards.concat([
        31004, 31027, 31028, 31036, 31039,
      ]);
    if (setIdentifiers.includes("ms"))
      drawEconomyCards = drawEconomyCards.concat([33004]);
    var influenceUsed = CountInfluence(identityCard, cardsAdded);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
    ...
    var otherCards = [];
    if (setIdentifiers.includes("sg"))
      otherCards = otherCards.concat([
        30002, 30003, 30004, 30005, 30006, 30007, 30008, 30009, 30011, 30012,
        30013, 30014, 30015, 30016, 30017, 30018, 30020, 30021, 30022, 30023,
    ...
    influenceUsed = CountInfluence(identityCard, cardsAdded);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        otherCards,
    ...
    var agendaCards = [];
    if (setIdentifiers.includes("sg"))
      agendaCards = agendaCards.concat([
        30060, 30044, 30036, 30067, 30068, 30069, 30070, 30052,
      ]);
    if (setIdentifiers.includes("su21"))
      agendaCards = agendaCards.concat([
        31041, 31051, 31052, 31061, 31062, 31071, 31072, 31073,
      ]);
    cardsAdded = cardsAdded.concat(
    ...
    if (setIdentifiers.includes("sg"))
      economyCards = economyCards.concat([
        30037, 30048, 30056, 30064, 30071, 30075,
      ]);
    if (setIdentifiers.includes("su21"))
      economyCards = economyCards.concat([31042, 31057, 31080, 31082]);
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
        economyCards,
    ...
    var iceCards = [];
    if (setIdentifiers.includes("sg"))
      iceCards = iceCards.concat([
        30038, 30062, 30039, 30046, 30054, 30047, 30072, 30063, 30055, 30073,
        30074,
      ]);
    if (setIdentifiers.includes("su21")) {
      iceCards = iceCards.concat([
        31043, 31044, 31046, 31055, 31056, 31065, 31066, 31067, 31076, 31077,
        31081,
      ]);
    ...
    if (setIdentifiers.includes("sg"))
      otherCards = otherCards.concat([
        30040, 30041, 30042, 30045, 30049, 30050, 30053, 30058, 30061, 30066,
      ]);
    if (setIdentifiers.includes("su21")) {
      otherCards = otherCards.concat([
        31047, 31048, 31049, 31053, 31054, 31058, 31059, 31063, 31064, 31068,
        31069, 31079,
      ]);
      ...
      if (
        agendaDistribution[3] >=
        agendaDistribution[1] + agendaDistribution[0]
      )
        otherCards.push(31078);
    }
    cardsAdded = cardsAdded.concat(
      DeckBuildRandomly(
        identityCard,
