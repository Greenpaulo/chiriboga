  ...
  <script>
    ...
    (function() {
      ...
      function ShowOpponentDeckLightbox() {
        ...
        var oppJSON;
        try {
          oppJSON = JSON.parse(LZString.decompressFromEncodedURIComponent(compressed));
        } catch (e) {}
        ...
        for (var cid in counts) {
          if (cardSet[cid]) {
            ...
            (grouped[ct] = grouped[ct] || []).push(cid);
          }
        }
        ...
        for (var gi = 0; gi < order.length; gi++) {
          ...
          grouped[g].sort(function(a, b) {
            return cardSet[a].title.localeCompare(cardSet[b].title);
          });
          lines.push('[' + g.toUpperCase() + ']');
          for (var k = 0; k < grouped[g].length; k++) {
            var id = grouped[g][k];
            lines.push(counts[id] + ' ' + cardSet[id].title);
          }
          lines.push('');
        }
        ...
        if (savedJson) {
          ...
          if (saved && Array.isArray(saved.customSets) && saved.customSets.length > 0) {
            ...
            for (var i = 0; i < saved.customSets.length; i++) {
              ...
              if (code !== 'sg' && setFileMap[code] && setsToLoad.indexOf(setFileMap[code]) === -1) {
  ...
  <script>
      ...
      try {
        ...
        if (cardData && cardData.data) {
          for (var i = 0; i < cardData.data.length; i++) {
            ...
            window.cardCodeLookup[c.code] = {
              title: c.title || c.stripped_title,
    ...
    function RecalculateDeckCounts() {
      ...
      for (var i = 0; i < json.cards.length; i++) {
        var id = json.cards[i];
        if (typeof deckCounts[id] === 'undefined') deckCounts[id] = 1;
        else deckCounts[id]++;
      }
      UpdateCardCountsUI();
    }
    ...
    function AttachCardListEvents() {
      $("#cardcontainer .add-btn").off('click').on('click', function() {
        AddCardToDeck(parseInt($(this).attr('data-id')));
      });
      $("#cardcontainer .remove-btn").off('click').on('click', function() {
        RemoveCardFromDeck(parseInt($(this).attr('data-id')));
      });
    ...
    if (typeof setRegistry !== 'undefined' && setRegistry.availableSets) {
      for (var setKey in setRegistry.availableSets) {
        ...
        if (set && set.code && loadedSetCodes.indexOf(set.code) !== -1) {
          ...
          filterLabels[set.code] = customSetFilterLabels[set.code] || (set.name || set.code).toUpperCase();
        }
      }
    } else {
      // Fallback if setRegistry not available
    ...
    function UpdateLaunchStrings() {
      ...
      if (deckModified) {
        deckForUri = {
          identity: parseInt(json.identity),
    ...
    function GenerateDeck() {
      ...
      $("#deck").on("input propertychange paste", function() {
        MarkDeckModified();
        Parse();
      });
    ...
    function InitMain() {
            ...
            try {
              ...
              if (identityCode && window.cardCodeLookup && window.cardCodeLookup[identityCode]) {
                ...
                function normalizeTitle(str) {
                  if (!str) return '';
                  try {
                    str = str.normalize ? str.normalize('NFKC') : str;
                  } catch (e) {}
      ...
      function ImportJSDeckFromText(text) {
        try {
          ...
          if (!callMatch) {
            alert('Invalid JS deck file format.');
            return;
          }
          ...
          if (!identityMatch || !cardsMatch) {
            alert('Missing identity or cards in deck file.');
            return;
          }
          ...
          while ((m = lineRegex.exec(cardsBlock)) !== null) {
            ...
            if (qty > 0 && qty <= 99) { // reasonable bound
              ...
              if (totalEntries > 1000) {
                alert('Too many card entries.');
                return;
              }
            }
          }
          if (!totalEntries) {
            alert('No cards found in deck file.');
            return;
          }
    ...
    function Normalise(src) {
      if (!src) return "";
      try {
        src = src.normalize ? src.normalize('NFKC') : src;
      } catch (e) {}
