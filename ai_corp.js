...
class CorpAI {
  ...
  _iceInCardsCommon(
    ...
    for (var i = 0; i < cards.length; i++) {
      if (cards[i].cardType == "ice") {
        if (extraCost < corp.creditPool) {
          //even not-affordable ice is excluded if it can't be installed
          var yesToThisIce = true;
          if (typeof cards[i].AIWorthwhileIce == "function") {
            yesToThisIce = cards[i].AIWorthwhileIce.call(
              cards[i],
              serverToInstallTo,
              "install",
            );
          ...
          var isAffordable =
            cards[i].rezCost + extraCost <= corp.creditPool && yesToThisIce;
  ...
  _bestServerToUpgrade(upgrade) {
    ...
    if (
      ...
      this._protectionScore(corp.RnD, {}) <
        this._protectionScore(preferredServer, {})
    ) {
      if (
        ...
        if (
          this._archivesIsBackdoorToHQ() &&
          this._protectionScore(corp.archives, {}) <
            this._protectionScore(corp.HQ, {})
        )
          preferredServer = corp.archives;
  ...
  _scoringWindow(server) {
    var ret = 0;
    ret =
      this._protectionScore(server, {}) -
      this._protectionScore(corp.HQ, {
        returnArchivesLowerScoreForHQIfBackdoor: true,
      }) +
      0.3 * this._clicksLeft() +
      Credits(corp) / (Credits(runner) + 1.0) -
      1.0;
    return ret;
  }
    ...
    if (CheckCardType(card, ["upgrade"])) {
      ...
      if (server) {
        ...
        if (typeof card.AILimitPerServer == "function") {
          ...
          if (
            numberThatWouldBeInServer > card.AILimitPerServer.call(card, server)
          )
            return false;
        }
      }
  ...
  _reducedDiscardList(
    ...
    while (optionList.length > maxCount)
      optionList.splice(
        optionList.indexOf(this._bestNonAgendaTutorOption(optionList)),
        1,
      ); //the 1 means remove 1 card
  ...
  _reduceOptionsToBestCardToReturnToRnD(optionList) {
    var logStart =
      "Best card to return to R&D from " +
      JSON.stringify(CardsInOptionList(optionList));
    //if HQ is not protected or there is more than one agenda in hand, return agendas to R&D
    var returnAgendasToRnD = false;
    if (this._agendasInHand() > 1 || this._serverToProtect() == corp.HQ)
      returnAgendasToRnD = true;
    if (returnAgendasToRnD) {
      for (var i = 0; i < optionList.length; i++) {
        if (optionList[i].card.cardType == "agenda")
          optionList = [optionList[i]];
      }
    }
  ...
  _bestSabotageOption(optionList) {
    ...
    this._log(
      "considering sabotage options, need to trash at least " +
        minSabotageFromHQ +
        " from HQ",
    );
    ...
    fauxOptionList = this._reducedDiscardList(
      fauxOptionList,
      minSabotageFromHQ,
      selectCards.length,
    ); //min is minSabotageFromHQ, max is selectCards.length
  ...
  _advancementLimit(card, server) {
    ...
    if (
      server &&
      typeof card.advancementRequirement !== "undefined" &&
      ret == card.advancementRequirement
    ) {
      ...
      if (sanSan) {
        ...
        if (
          ...
          AvailableCredits(corp) >=
            RezCost(sanSan) + ret - Counters(card, "advancement")
        )
          ret--;
      }
    }
  ...
  _bestNonAgendaTutorOption(optionList, onlyTheBest = false) {
    ...
    for (var i = 0; i < optionList.length; i++) {
      if (optionList[i].card) {
        if (optionList[i].card.AIFastAdvance)
          fastAdvanceCards.push(optionList[i].card);
      }
    }
    if (fastAdvanceCards.length > 0)
      return this._firstOptionWithCard(
        optionList,
        this._highestELO(fastAdvanceCards),
      );
    ...
    for (var i = 0; i < optionList.length; i++) {
      if (optionList[i].card && !optionList[i].card.AIIsRecurOrTutor) {
        if (
          optionList[i].card.AITagPunishment ||
          optionList[i].card.AIDamageOperation ||
          (typeof optionList[i].card.AIWouldPlay == "function" &&
            optionList[i].card.AIWouldPlay.call(optionList[i].card))
        )
          opportunityCards.push(optionList[i].card);
      }
    }
    if (opportunityCards.length > 0)
      return this._firstOptionWithCard(
        optionList,
        this._highestELO(opportunityCards),
      );
    ...
    for (var i = 0; i < optionList.length; i++) {
      if (optionList[i].card && !optionList[i].card.AIIsRecurOrTutor)
        cardList.push(optionList[i].card);
    }
    if (!this._affordableIce(null).length == 0) {
      var affordableIceInList = this._affordableIce(null, cardList);
      if (affordableIceInList.length > 0)
        return this._firstOptionWithCard(
          optionList,
          this._highestELO(affordableIceInList),
        );
    }
  ...
  _bestRecurToHQOption(
    optionList,
    serverUnderThreat,
    useNowIfPossible = false,
  ) {
    ...
    var faai = this._fullyAdvanceableAgendaInstalled(
  ...
  _potentialDamageOnBreach(server) {
    ...
    if (
      this._agendaPointsInServer(server) > 0 &&
      corp.identityCard.title == "Jinteki: Personal Evolution"
    ) {
    ...
    for (var i = 0; i < corp.scoreArea.length; i++) {
      if (
        corp.scoreArea[i].title == "House of Knives" &&
        !corp.scoreArea[i].usedThisRun
      )
        ret++;
    }
    ...
    if (
      this._copyOfCardExistsIn("Hokusai Grid", server.root) &&
      creditsLeft > 1
    ) {
      creditsLeft -= 2;
      ret++;
  ...
  //returns int
  _agendasInServer(server, ignoreCards = []) {
    if (!server) return 0;
    var ret = 0;
    for (var i = 0; i < server.root.length; i++) {
      if (
        CheckCardType(server.root[i], ["agenda"]) &&
        !ignoreCards.includes(server.root[i])
      )
        ret++;
    }
    if (typeof server.cards !== "undefined") {
      for (var i = 0; i < server.cards.length; i++) {
        if (
          CheckCardType(server.cards[i], ["agenda"]) &&
          !ignoreCards.includes(server.cards[i])
        )
          ret++;
      }
    }
  ...
  _HVTserver() {
    ...
    for (var i = 0; i < corp.remoteServers.length; i++) {
      for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
        if (this._isHVT(corp.remoteServers[i].root[j]))
  ...
  //optionally, specific an array of cards to ignore
  _agendasInHand(ignoreCards = []) {
    //returns int
    return this._agendasInServer(corp.HQ, ignoreCards);
  }

  //optionally, specific an array of cards to ignore
  _HVTsInHand(ignoreCards = []) {
    //returns int
    var ret = 0;
  ...
  _bluffAdvanceLimit(card) {
    ...
    if (typeof card.AIAdvancementLimit == "function")
      advlim = card.AIAdvancementLimit.call(card);
    var hostileCards = corp.RnD.cards
      .concat(corp.HQ.cards)
      .concat(corp.archives.cards);
    for (var k = 0; k < hostileCards.length; k++) {
      if (CheckSubType(hostileCards[k], "Hostile")) {
        if (typeof hostileCards[k].AIAdvancementLimit == "function") {
          var hostadvlim = hostileCards[k].AIAdvancementLimit.call(
            hostileCards[k],
          );
  ...
  _obsoleteBluff(card) {
    ...
    if (typeof card.AITurnsInstalled !== "undefined") {
      ...
      if (
        agendasInHand > 0 &&
        corp.HQ.cards.length - this._agendasInHand() < agingFactor
      ) {
        if (!CheckSubType(card, "Hostile") && CheckCardType(card, ["asset"])) {
          ...
          if (
            this._remoteServerWithHighestIceAndRootProtection() ==
            GetServer(card)
          )
            return true;
  ...
  _isHostedVirusBreaker(hosted, iceCard) {
  ...
  _breakerBreakCost(breaker) {
    var match =
      /(\d+)\s*(?:\[c\]|credits?)\s*:[^.]*?break\s+(?:up to\s+)?(\d+)/i.exec(
        breaker.cardText,
      );
    if (match != null) {
  ...
  _protectionScore(
    ...
    if (!options.ignoreSuccessfulRuns) {
      ...
      if (
        ...
        archivesIsBackdoorToHQ &&
        corp.HQ.AISuccessfulRuns !== "undefined"
      ) {
        if (corp.HQ.AISuccessfulRuns > successfulRuns)
          successfulRuns = corp.HQ.AISuccessfulRuns;
      }
  ...
  _bestProtectedRemote() {
    //returns the remote server that has the highest protection
  ...
  _emptyProtectedRemotes() {
    //returns a list (strongest protection first) of remote servers which are empty (can contain upgrades or obsolete bluffs though) and have ice in front
    var ret = [];
    var protectionScores = [];
    for (var i = 0; i < corp.remoteServers.length; i++) {
      var hasRoom = true;
      for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
        if (CheckCardType(corp.remoteServers[i].root[j], ["agenda", "asset"])) {
          ...
          //but keep if useful for Trick of Light
            ...
            CheckCounters(corp.remoteServers[i].root[j], "advancement", 2) &&
            this._copyOfCardExistsIn(
              "Trick of Light",
              corp.HQ.cards.concat(corp.RnD.cards),
            )
          )
            hasRoom = false;
        }
  ...
  _economyCards(affordableOnly = false) {
    var economyCards = [];
    if (!affordableOnly || corp.creditPool >= 3)
      economyCards.push("Celebrity Gift"); //we have this first because playing other cards first would reduce cards in hand
    var subliminal = this._copyOfCardExistsIn(
      "Subliminal Messaging",
      AllCards(corp),
    );
    if (subliminal && !subliminal.copyPlayedThisTurn)
      economyCards.push("Subliminal Messaging"); //this is next priority because it gives the click back
    if (!affordableOnly || corp.creditPool >= 10)
      economyCards.push("Government Subsidy");
    if (!affordableOnly || corp.creditPool >= 5)
      economyCards.push("Hedge Fund");
    if (
      (!affordableOnly || corp.creditPool >= 5) &&
      this._agendasInHand() < corp.HQ.cards.length - 1
    )
      economyCards.push("Hansei Review"); //only if there is at least 1 non-agenda card (other than this) in HQ
    if (!affordableOnly || corp.creditPool >= 2)
      economyCards.push("Marilyn Campaign");
    if (!affordableOnly || corp.creditPool >= 3)
      economyCards.push("Regolith Mining License");
    if (!affordableOnly || corp.creditPool >= 2)
  ...
  _bestMainPhaseEconomyOption(optionList) {
    ...
    if (optionList.indexOf("advance") > -1) {
      var oaktown = this._copyOfCardExistsIn(
        "Oaktown Renovation",
        installedCards,
      );
      if (oaktown)
      ...
      for (var i = 0; i < corp.HQ.cards.length; i++) {
        if (economyCards[j] == GetTitle(corp.HQ.cards[i])) {
          if (
            ...
            this._log(
              GetTitle(corp.HQ.cards[i]) + " might be good economy play?",
            );
            ...
            if (corp.HQ.cards[i].cardType != "upgrade") {
                ...
                if (installPreference > -1) {
                  ...
                  //new server
                    ...
                    prefToReturn = {
                      cardToInstall: corp.HQ.cards[i],
                      serverToInstallTo:
                        emptyProtectedRemotes[installPreference],
                    };
                }
              }
              ...
              if (prefToReturn != null) {
                this._log(
                  GetTitle(corp.HQ.cards[i]) +
                    " might be good economy install?",
                );
                return this._returnPreference(
                  optionList,
                  "install",
                  prefToReturn,
                );
    ...
    if (MaxHandSize(corp) - PlayerHand(corp).length > 0) {
      ...
      if (corp.HQ.cards.length < corp.creditPool)
        drawCards.push("Predictive Planogram"); //simple check whether to use for draw or save for econ
  ...
  _iceInstallOptions(serverToInstallTo, cards, priorityOnly) {
    ...
    if (this._HVTsInstalled() == 0 && this._clicksLeft() > 1)
      permitNewEmpty = true;
  ...
  _rankedInstallOptions(
    ...
    if (emptyProtectedRemotes.length > 0)
      strongestEmptyRemote = emptyProtectedRemotes[0];
    ...
    for (var i = 0; i < corp.HQ.cards.length; i++) {
      ...
      if (CheckCardType(card, ["agenda"])) {
        if (AgendaPoints(corp) + card.agendaPoints >= AgendaPointsToWin()) {
          if (
            this._advancementLimit(card, strongestEmptyRemote) <=
          ...
          if (installPreference > -1) {
            if (installPreference > emptyProtectedRemotes.length - 1)
              ret.push({
                cardToInstall: cards[i],
                serverToInstallTo: null,
                reason: "AIWorthInstalling returned new server",
              });
            //new server
            else
              ret.push({
    ...
    if (
      serverToInstallTo == null ||
      (serverToInstallTo.root.length < 1 && this._agendasInHand() < 1)
    )
    ...
    for (var i = 0; i < upgradeInstallPreferences.length; i++) {
      upgradeInstallPreferences[i].reason =
        "returned by _upgradeInstallPreferences";
    }
    ...
    this._log(
      "Scoring windows for empty servers: " + JSON.stringify(scoringWindows),
    );
    ...
    intoServerOptions.sort(function (a, b) {
      var aAdvReq = corp.AI._advancementLimit(
        a.cardToInstall,
        a.serverToInstallTo,
      );
      var bAdvReq = corp.AI._advancementLimit(
        b.cardToInstall,
        b.serverToInstallTo,
      );
      var aWindow = 0;
      var bWindow = 0;
      for (var i = 0; i < scoringServers.length; i++) {
        if (scoringServers[i] == a.serverToInstallTo)
          aWindow = scoringWindows[i];
    ...
    if (upgradeInstallPreferences.length > 0 && intoServerOptions.length > 0) {
      if (
        upgradeInstallPreferences[0].serverToInstallTo ==
        intoServerOptions[0].serverToInstallTo
      ) {
        if (upgradeInstallPreferences[0].cardToInstall.AIIsScoringUpgrade)
          preferUpgrade = true;
      }
    }
    ...
    if (
      snare &&
      !PlayerCanLook(runner, snare) &&
      AvailableCredits(corp, "using", snare) > 3
    )
      ret.push({
        cardToInstall: snare,
        serverToInstallTo: strongestEmptyRemote,
  ...
  _runnerMayWinIfServerBreached(server) {
    if (!server) return false;
    if (
      AgendaPoints(runner) + this._agendaPointsInServer(server) >=
      AgendaPointsToWin()
    )
      return true;
    return false;
  ...
  _iceWorthRezzing(card, currentRezCost, server) {
      ...
      for (var i = thisIcePosition - 1; i > -1; i--) {
        if (!server.ice[i].rezzed) {
          iceToCompareList.push({
            card: server.ice[i],
            server: server,
            cost: RezCost(server.ice[i]),
            value: thisServerValue,
          });
        }
      }
      ...
      for (var i = 0; i < serversToCompare.length; i++) {
        ...
        for (var j = 0; j < serverToCompare.ice.length; j++) {
          if (!serverToCompare.ice[j].rezzed) {
            iceToCompareList.push({
              card: serverToCompare.ice[j],
              server: serverToCompare,
              cost: RezCost(serverToCompare.ice[j]),
      ...
      if (CheckCredits(corp, rezCostToCompare, "rezzing")) {
        ...
        if (!CheckCredits(corp, currentRezCost + rezCostToCompare, "rezzing")) {
          ...
          if (
            ...
            this._log(
              "Rez cost not worth it, need to save it for " +
                iceToCompare.title +
                " in " +
                ServerName(serverToCompare),
            );
            rezIce = false;
          } else
            this._log(
              "Rez this is better than " +
                iceToCompare.title +
                " in " +
                ServerName(serverToCompare),
            );
    ...
    for (var i = 0; i < server.root.length; i++) {
      if (!server.root[i].rezzed) {
        if (
          typeof server.root[i].AIDefensiveValue == "function" &&
          server.root[i].AIDefensiveValue.call(server.root[i], server) > 0
        ) {
          if (
            !CheckCredits(
              corp,
              currentRezCost + RezCost(server.root[i]),
              "rezzing",
            )
          ) {
            if (
              this._cardProtectionValue(server.root[i]) > thisIceProtectionValue
            ) {
              this._log(
                "Rez cost not worth it, need to save it for " +
                  server.root[i].title +
    ...
    if (runner.resolvingCards.length > 0) {
      var copyOfInsideJob = this._copyOfCardExistsIn(
        "Inside Job",
      ...
      if (copyOfInsideJob) {
        if (!copyOfInsideJob.encounteredIceThisRun) {
          ...
          if (
            indexInServer == 0 ||
            (!server.ice[indexInServer - 1].rezzed &&
              corp.creditPool <
                RezCost(card) + RezCost(server.ice[indexInServer - 1]))
          ) {
            if (
              typeof card.automaticOnRez == "undefined" &&
      ...
      for (var i = 0; i < cardsToRezEOT.length; i++) {
        ...
        if (copyOfCard) {
          ...
          if (typeof copyOfCard.AIWouldTrigger == "function") {
            if (!copyOfCard.AIWouldTrigger.call(copyOfCard)) wouldUse = false;
          }
          if (wouldUse)
            return this._returnPreference(optionList, "rez", {
              cardToRez: copyOfCard,
  ...
  _potentialOperationDamageDirections(point) {
    ...
    if (point.corpCredits > 2 && point.corpClicks > 0) {
      ...
      if (indexinhand > -1) {
        ...
        newarchivescards = newarchivescards.concat(
          newhandcards.splice(indexinhand, 1),
        ); //move played card
        ret.push({
          ...
          damageSoFar:
            point.damageSoFar + cardinhand.printedAgendaPointsThisTurn,
          using: cardinhand,
    ...
    cardinhand = this._copyOfCardExistsIn(
      "Punitive Counterstrike",
      point.handCards,
    );
    ...
    var punitiveCost = 3;
    if (AgendaPointsToWin() - AgendaPoints(runner) > 3)
      punitiveCost += Link() + Credits(runner) - 4; //the 4 assumes base strength 5
    if (point.corpCredits >= punitiveCost && point.corpClicks > 0) {
      ...
      if (indexinhand > -1) {
        ...
        ret.push({
          ...
          archivesCards: newarchivescards,
          damageSoFar:
            point.damageSoFar + cardinhand.printedAgendaPointsLastTurn,
          using: cardinhand,
          persist: point.persist,
        });
      }
    ...
    if (point.corpClicks > 1) {
      ...
      if (indexinhand > -1) {
        ...
        for (var i = 0; i < cardsToRecur.length; i++) {
          var cardinarchives = this._copyOfCardExistsIn(
            cardsToRecur[i],
            point.archivesCards,
          );
          ...
          if (indexinarchives > -1) {
            ...
            newarchivescards = newarchivescards.concat(
              newhandcards.splice(indexinhand, 1),
            ); //move archived memories (to back so it doesn't change indexinarchives)
            newhandcards = newhandcards.concat(
              newarchivescards.splice(indexinarchives, 1),
            ); //move recurred card
            ret.push({
  ...
  _potentialOperationDamageThisTurn(output = []) {
    ...
    var startingDirections =
      this._potentialOperationDamageDirections(startingPoint);
    var todo = [];
    ...
    while (todo.length > 0 && loops < maxLoops) {
      ...
      var directions = this._potentialOperationDamageDirections(
      ...
      if (directions.length > 0) {
        ...
        if (bestpath.length < 1) better = true;
          ...
          if (thisendpoint.damageSoFar > bestendpoint.damageSoFar)
            better = true;
          else if (thisendpoint.damageSoFar == bestendpoint.damageSoFar) {
            if (thisendpoint.handCards.length > bestendpoint.handCards.length)
              better = true;
            else if (
              thisendpoint.handCards.length == bestendpoint.handCards.length
            ) {
              if (
                clickValueScale * thisendpoint.corpClicks +
                  thisendpoint.corpCredits >
                clickValueScale * bestendpoint.corpClicks +
                  bestendpoint.corpCredits
              )
                better = true;
            }
          }
  ...
  _potentialAdvancementDirections(point, options = null) {
    ...
    if (point.corpCredits > 0 && point.corpClicks > 0) {
      ...
      if (
        point.advancementSoFar < 1 &&
        corp.identityCard.title == "Weyland Consortium: Built to Last"
      )
        newCredits += 2;
    ...
    if (point.corpCredits > 1 && point.corpClicks > 0) {
      ...
      if (pgidx > -1) {
        ...
        if (maxX + point.advancementSoFar > limit)
          maxX = limit - point.advancementSoFar;
        ...
        for (var i = 0; i < advCards.length; i++) {
          ...
          point.persist.forEach(function (item) {
            ...
            if (typeof item.trickOfLightSource != "undefined") {
              if (item.trickOfLightSource == advCards[i].card)
                advRemaining -= 2;
            }
          });
          if (advRemaining > 1) {
  ...
  _potentialAdvancement(
    ...
    //generate starting directions as todo
    var startingDirections = this._potentialAdvancementDirections(
      startingPoint,
      options,
    );
    var todo = [];
    ...
    while (todo.length > 0 && loops < maxLoops) {
      loops++;
      var thispath = todo.shift();
      var directions = this._potentialAdvancementDirections(
        thispath[thispath.length - 1],
        options,
      );
      if (directions.length > 0) {
        ...
        if (bestpath.length < 1) better = true;
          ...
          var thisendpoint = thispath[thispath.length - 1];
          if (
            thisendpoint.advancementSoFar > bestendpoint.advancementSoFar &&
            bestendpoint.advancementSoFar < limit
          )
            better = true;
          else if (
            thisendpoint.advancementSoFar >= bestendpoint.advancementSoFar ||
            thisendpoint.advancementSoFar >= limit
          ) {
            if (thisendpoint.handCards.length > bestendpoint.handCards.length)
              better = true;
            else if (
    ...
    if (loops == maxLoops)
      console.log("Advancement search exceeded loop limit");
    //all paths found, return best
    var ret = 0;
    if (bestpath.length > 0)
      ret =
        bestpath[bestpath.length - 1].advancementSoFar - startingAdvancement;
    for (var i = 0; i < bestpath.length; i++) {
      output.push(bestpath[i].using);
    }
    return ret;
  ...
  _isFullyAdvanceableHostileAsset(card, fastAdvanceArray) {
    ...
    if (typeof fastAdvanceArray == "undefined")
      fastAdvanceArray = corp.HQ.cards.concat(corp.resolvingCards); //source of fast advance cards
    if (!CheckCardType(card, ["asset"]) || !CheckSubType(card, "Hostile"))
      return false;
    if (typeof card.AIRushToFinish == "function") {
      if (card.AIRushToFinish.call(card)) {
        return (
          this._potentialAdvancement(card, Infinity, true, fastAdvanceArray) >=
          this._advancementStillRequired(card)
        );
      }
    }
    return false;
  ...
  _isFullyAdvanceableAgenda(card, fastAdvanceArray) {
    //can be finished and scored this turn
    if (typeof fastAdvanceArray == "undefined")
      fastAdvanceArray = corp.HQ.cards.concat(corp.resolvingCards); //source of fast advance cards
  ...
  _fullyAdvanceableAgendaInstalled(fastAdvanceArray) {
    var installedCards = InstalledCards(corp);
    for (var i = 0; i < installedCards.length; i++) {
      if (this._isFullyAdvanceableAgenda(installedCards[i], fastAdvanceArray))
        return true;
    }
    return false;
  }
  ...
  _commonCardToPlayChecks(
    cardToPlay,
    msg = "",
    logwillplay = false,
    checkWouldPlay = true,
    requireActionPhase = true,
  ) {
    if (cardToPlay) {
      ...
      if (checkWouldPlay && typeof cardToPlay.AIWouldPlay == "function")
        playThis = cardToPlay.AIWouldPlay.call(cardToPlay);
  ...
  Phase_Main(optionList) {
    ...
    if (optionList.includes("play")) {
      ...
      this._log(
        "I could do " +
          opDamageThisTurn +
          " damage with these cards: " +
          JSON.stringify(damageOps),
      );
    ...
    if (optionList.indexOf("advance") > -1) {
      ...
      for (var i = 0; i < corp.remoteServers.length; i++) {
        for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
          if (
            CheckAdvance(corp.remoteServers[i].root[j]) &&
            this._isFullyAdvanceableAgenda(corp.remoteServers[i].root[j])
          ) {
            if (almostDoneAgenda) {
              //assume the one with the higher printed advancement requirement or agendaPoints is better
              if (
                this._advancementLimit(
                  corp.remoteServers[i].root[j],
                  corp.remoteServers[i],
                ) >
                  this._advancementLimit(almostDoneAgenda, almostDoneServer) ||
                corp.remoteServers[i].root[j].agendaPoints >
                  almostDoneAgenda.agendaPoints
              ) {
      ...
      for (var i = 0; i < corp.remoteServers.length; i++) {
        for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
          if (
            CheckAdvance(corp.remoteServers[i].root[j]) &&
            this._isFullyAdvanceableHostileAsset(corp.remoteServers[i].root[j])
          ) {
    ...
    if (
      ...
      for (var i = 0; i < corp.remoteServers.length; i++) {
        for (var j = 0; j < corp.remoteServers[i].root.length; j++) {
          ...
          if (CheckAdvance(card) && !this._obsoleteBluff(card)) {
            ...
            var shouldStartAdvancingAgenda =
              thisRemoteProtectionScore > 0.5 * Credits(runner) ||
              thisRemoteProtectionScore >
                this._protectionScore(corp.HQ, {
                  returnArchivesLowerScoreForHQIfBackdoor: true,
                });
            ...
            if (card.advancement > 0)
              startOrContinueAdvancement = true; //already started, feel free to continue (the counter shows the runner it is advanceable)
            else if (
              !CheckCardType(card, ["agenda"]) ||
              card == almostDoneAgenda ||
              shouldStartAdvancingAgenda
            ) {
              if (
                this._potentialAdvancement(card, Infinity, false) >=
                advancementLimit
              )
            ...
            if (startOrContinueAdvancement) {
              if (
                ...
                if (potentialAdvCards.length > 0 && potentialAdvCards[0]) {
                  ...
                  if (!cardToPlay) {
                    var advancementRemaining =
                      this._advancementStillRequired(card);
                    var spareCredAfterExpensiveAdv =
                      Credits(corp) - (4 + advancementRemaining); //4 here is Biotic Labor (only expensive fast advance currently implemented)
                    //first check can even afford it
                    if (spareCredAfterExpensiveAdv >= 0) {
                      var worthExpensiveFastAdvance =
                        advancementRemaining > corp.clickTracker;
                      if (!worthExpensiveFastAdvance) {
                        ...
                        var copyOfPublicTrail = this._copyOfCardExistsIn(
                          "Public Trail",
                          arrayToCheckForCombo,
                        );
                        if (
                          spareCredAfterExpensiveAdv >= 4 &&
                          card.title == "Orbital Superiority" &&
                          copyOfPublicTrail &&
                          copyOfPublicTrail.successfulRunLastTurn
                        )
                          worthExpensiveFastAdvance = true;
                        //and after score:
                        ...
                        if (
                          ...
                          arrayToCheckForCombo = arrayToCheckForCombo.concat(
                            corp.archives.cards,
                          );
                        //when the agenda scored is Offworld Office, extra money exists
                        if (
                          card.title == "Offworld Office" ||
                          card.title == "Hostile Takeover"
                        )
                          spareCredAfterExpensiveAdv += 7;
                        //now check
                        if (
                          spareCredAfterExpensiveAdv >= 3 &&
                          this._copyOfCardExistsIn(
                            "Neurospike",
                            arrayToCheckForCombo,
                          )
                          ...
                          worthExpensiveFastAdvance = true;
                        if (
                          card.title == "Tomorrow's Headline" &&
                          this._potentialTagPunishment(
                            runner.tags + 1,
                            1,
                            spareCredAfterExpensiveAdv,
                          )
                        )
                          worthExpensiveFastAdvance = true;
                      }
                  ...
                  if (
                    (cardToPlay && card == almostDoneAgenda) ||
                    rushToFinish
                  ) {
                    //for now let's just use economy advance for finishing agendas or specific cards
        ...
        if (installedCards[i].canBeAdvanced) {
          ...
          if (CheckCardType(installedCards[i], ["ice"])) {
            ...
            if (typeof installedCards[i].AIAdvancementLimit == "function")
              advancementLimit = installedCards[i].AIAdvancementLimit.call(
                installedCards[i],
              );
    ...
    this._log(
      "Nothing to advance" + (sufficientEconomy ? "" : " (can't afford it)"),
    );

    if (optionList.indexOf("install") != -1) {
      var priorityRankedInstallOptions = this._rankedInstallOptions(
        corp.HQ.cards,
        true,
      ); //the true includes priority options only (e.g. exclude ice you can't afford or non-ice unprotected)
      if (priorityRankedInstallOptions.length > 0)
        return this._returnPreference(
          optionList,
          "install",
          priorityRankedInstallOptions[0],
        );
    }
      ...
      if (optionList.indexOf("install") != -1) {
        ...
        if (rankedInstallOptions.length > 0) {
          ...
          if (
            ...
            this._protectionScore(
              rankedInstallOptions[0].serverToInstallTo,
              {},
            ) > 1 ||
            (this._clicksLeft() > 2 &&
              this._affordableIce(
  ...
  Choice(optionList, choiceType) {
    ...
    if (ret < 0 && choiceType == "select") {
      if (executingCommand == "trash") ret = this._bestTrashOption(optionList);
        ...
        if (ret < 0) {
          //none found in options list? I guess we just need to return a default (but investigate, this isn't ideal)
    ...
    if (ret < 0) {
      ...
      //opportunity to act at end of runner turn
        ...
        (currentPhase.identifier == "Corp 2.2" ||
          currentPhase.identifier == "Corp 2.1") &&
    ...
    if (ret < 0) {
      ...
      this._log(
        "I don't have code to handle this situation: " +
          JSON.stringify([
            currentPhase.title,
